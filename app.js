...
let totall = 0; //total will hold the sum of all grades

for (let i = 0; i < myStudents.length; i++) {
	let student = myStudents[i];
	totall += student.grade; //add each grade to total
}
console.log(totall / myStudents.length); //divide by number of students
...
const targe = Math.floor(Math.random() * 10);
// Make initial guess
let gues = Math.floor(Math.random() * 10);

// Continue looping while guess is not the target num
while (gues !== targe) {
	console.log(`Target: ${targe} Guess: ${gues}`);
	// IMPORTANT!!
